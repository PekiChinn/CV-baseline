...
from Paper.GoogleNet_V1.tools.my_dataset import NCFMDataSet
from Paper.GoogleNet_V1.tools.common_tools import get_googlenet
...
if __name__ == "__main__":
    ...
    # 构建DataLoder
    train_loader = DataLoader(dataset=train_data, batch_size=BATCH_SIZE, num_workers=36, pin_memory=True, shuffle=True)
    valid_loader = DataLoader(dataset=valid_data, batch_size=4, num_workers=36, pin_memory=True)
    ...
    # flag = 1
    if flag:
        fc_params_id = list(map(id, googlenet_model.fc.parameters()))  # 返回的是parameters的 内存地址
