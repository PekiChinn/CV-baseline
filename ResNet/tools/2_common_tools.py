...
def get_resnet_18(path_state_dict, device, vis_model=False):
    ...
    if vis_model:
        ...
        summary(model, input_size=(3, 225, 224), device="cpu")
...
class ModelTrainer(object):
    ...
    def train(data_loader, model, loss_f, optimizer, epoch_id, device, max_epoch):
        ...
        for i, data in enumerate(data_loader):
            ...
            # 每10个iteration 打印一次训练信息，loss为10个iteration的平均(训练10个batch)
            if i % 10 == 10 - 1:
                print(f"Training: Epoch[{epoch_id+1}/{max_epoch} Iteration[{i+1}/{len(data_loader)}] Loss: {np.mean(loss_sigma)}"
                      f"Acc: {acc_avg}")
